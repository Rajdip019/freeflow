import { useAuth } from "@/contexts/AuthContext";
import { useUserContext } from "@/contexts/UserContext";
import { IReviewImageData } from "@/interfaces/ReviewImageData";
import { Tooltip, Avatar } from "@chakra-ui/react";
import React from "react";
import SendInvitesIconModal from "../Modal/SendInvitesIconModal";
import ImageDeleteModalConfirmation from "@/components/Modal/ImageDeleteModalConfirmation";
import Copy from "../shared/Copy";
import { APP_URL } from "@/helpers/constants";
import PublicAndPrivate from "./PublicAndPrivate";

interface Props {
  isCommentsOn: boolean;
  setIsCommentsOn: React.Dispatch<React.SetStateAction<boolean>>;
  imageData: IReviewImageData;
  isCompareView: boolean;
  setIsCompareView: React.Dispatch<React.SetStateAction<boolean>>;
}

const ReviewImageToolbarAdmin: React.FC<Props> = ({
  isCommentsOn,
  setIsCommentsOn,
  imageData,
  isCompareView,
  setIsCompareView,
}) => {
  const { authUser } = useAuth();
  const { user } = useUserContext();
  return (
    <div className="flex h-16 items-center gap-5">
      <Tooltip label="Toggle Comments">
        <button
          onClick={() => {
            setIsCommentsOn(!isCommentsOn);
          }}
          disabled={isCompareView}
          className="text-gray-400 transition-all hover:text-gray-50 disabled:cursor-not-allowed disabled:text-gray-600 hover:disabled:disabled:text-gray-600"
        >
          {isCommentsOn ? (
            <svg
              fill="none"
              stroke="currentColor"
              strokeWidth={1.5}
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              className=" w-5"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88"
              />
            </svg>
          ) : (
            <svg
              fill="none"
              stroke="currentColor"
              strokeWidth={1.5}
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              className=" w-5"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"
              />
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
              />
            </svg>
          )}
        </button>
      </Tooltip>
      {imageData?.currentVersion && (
        <Tooltip label="Toggle Comparison View">
          <button onClick={() => setIsCompareView(!isCompareView)}>
            <svg
              className={`${
                isCompareView ? " text-white" : " text-gray-400"
              } w-6`}
              fill="currentColor"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
            >
              <path d="M5.566 4.657A4.505 4.505 0 016.75 4.5h10.5c.41 0 .806.055 1.183.157A3 3 0 0015.75 3h-7.5a3 3 0 00-2.684 1.657zM2.25 12a3 3 0 013-3h13.5a3 3 0 013 3v6a3 3 0 01-3 3H5.25a3 3 0 01-3-3v-6zM5.25 7.5c-.41 0-.806.055-1.184.157A3 3 0 016.75 6h10.5a3 3 0 012.683 1.657A4.505 4.505 0 0018.75 7.5H5.25z" />
            </svg>
          </button>
        </Tooltip>
      )}
      <PublicAndPrivate image={imageData} />
      <Copy value={`${APP_URL}/review-image/${imageData.id}`} />
      <SendInvitesIconModal image={imageData} />
      <ImageDeleteModalConfirmation image={imageData} />
      <>
        {authUser?.uid === imageData?.uploadedById && (
          <Tooltip label={user?.email}>
            <Avatar
              className="ring-1 ring-purple-500"
              src={user?.imageURL as string}
              name={user?.name}
              size={"sm"}
            />
          </Tooltip>
        )}
      </>
    </div>
  );
};

export default ReviewImageToolbarAdmin;
