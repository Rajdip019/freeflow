...
const ReviewCanvas: React.FC<ReviewCanvasProps> = ({ imageSrc, imageId, version, imageData, uname }) => {
            ...
            if (storageUsed <= user.storage) {
                try {
                    const storageRef = ref(
                        storage,
                        `reviewImages/${authUser?.uid}/public/${imageData.imageName}/comments/${imageData.imageName}-${Date.now()}.png`
                    );
                    let bytes: number = 0;
                    ...
                    toast({
                        title: "You have exceeded your storage limit.",
                        status: "error",
                        duration: 5000,
                        isClosable: true,
                        position: "bottom-right",
    ...
    return (
        <div className=' flex flex-col items-center'>
            ...
            <div className=' flex w-9/12 items-center gap-3'>
                ...
                />
                <button
                    className=" btn-p mb-4 py-2 px-2 rounded "
                    disabled={isLoading || !comment}
                    onClick={addThread}
                >
                    {isLoading ? <Spinner size={"xs"} /> : <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
