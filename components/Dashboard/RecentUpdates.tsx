...
const RecentUpdates = () => {
    ...
    return (
        <div className="bg-[#334155] w-full p-4 rounded">
            <h3 className=" font-sec text-lg text-white flex items-center">Recent Updates <p className=" ml-2 text-xs bg-gray-500 p-1 rounded-full w-6 flex justify-center items-center">{latestUpdatedImages.length}</p></h3> 
            <Collapse startingHeight={220} in={isExpanded}>
                {latestUpdatedImages.map((image, index) => {
                    return (
                        ...
                        >
                            <p className=" text-white text-sm w-8/12 ml-1 truncate">{image.imageName}</p>
                            <div className=" w-4/8 flex items-center gap-3">
                                <div className=" text-white text-sm bg-[#059669] whitespace-nowrap rounded-full flex justify-center font-sec px-4">
                                    {image.newUpdate}
                                </div>
                                <Link
